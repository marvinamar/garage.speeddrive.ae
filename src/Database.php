...
class Database {
    ...
    protected function addFieldValuePairs(array $pairs) {
        foreach($pairs as $field => $value) {
            array_push($this->fieldValuePairs, "`{$field}` = " . (is_string($value) && !in_array($value, array('NOW()', 'CURDATE()', 'NULL')) ? "'{$value}'" : $value));
        }
    }
    ...
    protected function addValues(array $values) {
        foreach($values as $value) {
            array_push($this->values, (is_string($value) && !in_array($value, array('NOW()', 'CURDATE()', 'NULL')) ? "'{$value}'" : $value));
        }
    }
    ...
    public function fetch($command) {
        return $this->getResults($command);
    }
    ...
    public function update() {
        ...
        // echo "UPDATE {$this->tableName} SET {$this->getFieldValuePairs()} WHERE {$this->getConditions()}";
